...
class campeonato_model extends CI_Model
{
	...
	public function setPParticipantesMundial($dado1 = null)
	{
		if($dado1 == NULL) return 0;
		if($this->db->insert('participante_mundial', $dado1))
		{
			return 1;
		}
		return 2;
	...
	public function setPParticipantesAfricano($dado1 = null)
	{
		if($dado1 == NULL) return 0;
		if($this->db->insert('participante_africano', $dado1))
		{
			return 1;
		}
		return 2;
	...
	public function EliminarParticipanteCampeonatoMundialAtual($id = null)
	{
		if($id != NULL){
			$this->db->where('id_participante_mundial', $id);
			if($this->db->delete('participante_mundial')):
				return 1;
			else:
				return 2;
			endif;
	...
	public function EliminarCampeonatoMundial55($id = null)
	{
		if($id != NULL){
			$this->db->where('id_campeonato_mundial', $id);
			if($this->db->delete('campeonato_mundial')):
				return 1;
			else:
				return 2;
			endif;
	...
	public function EliminarCampeonatoAfricano55($id = null)
	{
		if($id != NULL){
			$this->db->where('id_campeonato_africano', $id);
			if($this->db->delete('campeonato_africano')):
				return 1;
			else:
				return 2;
			endif;
	...
	public function EliminarCampeonatoNacional($id = null)
	{
		if($id != NULL){
			$this->db->where('id_campeonato', $id);
			if($this->db->delete('campeonato')):
				$this->db->where('id_campeonato', $id);
				if($this->db->delete('campeonato_nacional')):
					return 1;
				else:
					return 2;
				endif;
	...
	function verificarCampeonatoMundial55($id_mundial = null){
		$query = $this->db->query('select * from participante_mundial where id_mundial ='.$id_mundial)->result();
		return $query;
	}

	function verificarCampeonatoAfricano55($id_africano = null){
		$query = $this->db->query('select * from participante_africano where id_africano ='.$id_africano)->result();
		return $query;
	}

	public function atualizarParticipanteCampeonatoMundial($id, $dados = null)
	{

	    	if ($id == NULL) return 0;
				$this->db->where('id_participante_mundial',$id);
			if ($this->db->update('participante_mundial',$dados)) {
				return 1;
			}else{
				return 2;
			}
	...
	public function atualizarParticipanteCampeonatoQRcode($id, $dados = null)
	{
	...
	public function verificarParticipantesMundialBilhete($bilhete = null, $id_mundial = null)
	{
		$query = $this->db->query('select * from participante_mundial where n_bilhete like "%'.$bilhete.'%" and id_mundial ='.$id_mundial)->result();
	...
	//funcao para verificar bilhete verificarBilheteDuplicado
	public function verificarParticipantesAfricanoBilhete($bilhete = null, $id_africano = null)
	{
		$query = $this->db->query('select * from participante_africano where n_bilhete like "%'.$bilhete.'%" and id_africano ='.$id_africano)->result();
	...
	public function getAllParticipantesMundial($id)
	{
		$query = $this->db->query('select * from participante_mundial where id_mundial='.$id)->result();
		return $query;
	}

	public function getAllParticipantesAfricano($id)
	{
		$query = $this->db->query('select * from participante_africano where id_africano='.$id)->result();
		return $query;
	}

	public function getAllParticipantesMundialId($id)
	{
		...
		where id_mundial ='.$id)->result();
		return $query;
	}

	public function getAllParticipantesAfricanoId($id)
	{
		...
		where id_africano ='.$id)->result();
		return $query;
	}

	public function getAllMundial()
	{
		$query = $this->db->query('select * from campeonato_mundial where local <> "control"')->result();
		return $query;
	}

	// public function getAllMundial()
	// {
	// 	$query = $this->db->query('select * from campeonato_mundial
	// 	inner join participante_mundial.id_mundial = campeonato_mundial.id_campeonato_mundial')->result();
	// 	return $query;
	// }

	public function getAllAfricano()
	{
		$query = $this->db->query('select * from campeonato_africano')->result();
		return $query;
	}

	public function getAllNacional()
	{
		...
		where campeonato.id_campeonato in (select id_campeonato from campeonato)')->result();
		return $query;
	}

	public function getAllProvincial($idAssociacao)
	{
		$query = $this->db->query('select * from campeonato
		inner join campeonato_provincial on campeonato_provincial.id_campeonato = campeonato.id_campeonato
		where campeonato.controlTela = 4 and campeonato.id_associacao ='.$idAssociacao)->result();
		return $query;
	}

	function getMundialActual(){
		$query = $this->db->query('select id_mundial from camp_mundo_atual')->result()[0];
		return $query->id_mundial;
	}

	function getAfricanoActual(){
		$query = $this->db->query('select id_africano from camp_africano_atual')->result()[0];
		return $query->id_africano;
	}

	public function GravarMundial($dado1){
		if($dado1 == NULL) return 0;
		if($this->db->insert('Campeonato_mundial', $dado1))
		{
			$ultimo_id = $this->db->insert_id();
			return $ultimo_id;
		}
	...
	public function GravarAfricano($dado1){
		if($dado1 == NULL) return 0;
		if($this->db->insert('campeonato_africano', $dado1))
		{
			$ultimo_id = $this->db->insert_id();
			return $ultimo_id;
		}
	...
	public function atualizarMundialActual($id, $dados)
	{

	    	if ($id == NULL) return 0;
				$this->db->where('id',$id);
			if ($this->db->update('camp_mundo_atual',$dados)) {
				return 1;
			}else{
				return 2;
			}
	...
	public function atualizarAfricanoActual($id, $dados)
	{

	    	if ($id == NULL) return 0;
				$this->db->where('id',$id);
			if ($this->db->update('camp_africano_atual',$dados)) {
				return 1;
			}else{
				return 2;
			}
  ...
  public function setCampeonato($dados = null)
	...
	public function updateControlTelaNacional($id = null, $dados = null){
		$this->db->where('id_campeonato', $id);
		$this->db->update('campeonato', $dados);
	}

	//Busca se um id_campeonato existe na tabela participante campeonato
	public function verificarParticipanteNacional($id = null){
			$query = $this->db->query('select * from participante_campeonato where id_campeonato_fk ='.$id)->result();
			return $query;
	}

	public function updateNacionalAtual($id = null, $dados = null){
		$this->db->where('id', $id);
		$this->db->update('camp_nacional_atual', $dados);
	}

	public function updateProvincialAssociacaolAtual($id = null, $dados = null){
		$this->db->where('id', $id);
		$this->db->update('camp_provincial_atual', $dados);
	}
	...
	public function getCapNacionalInfo($id = null)
	{
		$campeonatos = $this->db->query('select * from campeonato as ca
		inner join campeonato_nacional cap on ca.id_campeonato = cap.id_campeonato
		inner join local_campeonato lca on lca.id_local_campeonato = ca.id_local_campeonato and ca.id_campeonato ='.$id)->result();
	...
	public function setAllParticipante4($id, $id2){
	...
	public function setAllParticipante($id){
		...
		if(empty($query)){
			...
			$idAssociacao = $query[0]->id_associacao_pro;
		}

		$participantes = $this->db->query('SELECT * FROM participante_campeonato
		INNER JOIN campeonato ON campeonato.id_campeonato = participante_campeonato.id_campeonato_fk
		WHERE campeonato.id_associacao = '.$idAssociacao.' and id_campeonato_fk ='.$id)->result();

		return $participantes;
	}

	public function setAllParticipanteByBilhete($id, $bilhete){
		$descricao = $this->session->userdata("nome");
		$query = $this->db->query("SELECT id_associacao_pro FROM associacao_provincia WHERE descricao_associacao = '$descricao'")->result();
		...
		$participantes = $this->db->query('SELECT * FROM participante_campeonato
		INNER JOIN campeonato ON campeonato.id_campeonato = participante_campeonato.id_campeonato_fk
		WHERE campeonato.id_associacao = '.$idAssociacao.' and id_campeonato_fk ='.$id.' and bilhete_identidade like "%'.$bilhete.'%"')->result();

		return $participantes;
	}

	public function getAllParticipanteTotal($id){

		$participantes = $this->db->query('SELECT count(*) as total FROM participante_campeonato
		INNER JOIN campeonato ON campeonato.id_campeonato = participante_campeonato.id_campeonato_fk
		WHERE id_campeonato_fk ='.$id)->result()[0];

		return $participantes->total;
	}

	public function getAllParticipanteTotalMasculino($id){

		$participantes = $this->db->query('SELECT count(*) as total FROM participante_campeonato
		INNER JOIN campeonato ON campeonato.id_campeonato = participante_campeonato.id_campeonato_fk
		WHERE genero = "Masculino" and id_campeonato_fk ='.$id)->result()[0];

		return $participantes->total;
	}

	public function getAllParticipanteTotalFemenino($id){

		$participantes = $this->db->query('SELECT count(*) as total FROM participante_campeonato
		INNER JOIN campeonato ON campeonato.id_campeonato = participante_campeonato.id_campeonato_fk
		WHERE genero = "Femenino" and id_campeonato_fk ='.$id)->result()[0];

		return $participantes->total;
	}

	public function setAllParticipante2($idcampeonato){
	 	...
	 	if($idcampeonato != 0){
			...
			foreach($query as $escalao2){
				...
				foreach($generos as $genero){
					...
					$categoriaPesos = $this->db->query('SELECT DISTINCT(categoriaPeso) FROM `participante_campeonato` WHERE id_campeonato_fk='.$idcampeonato.' and categoriaPeso != "NULL" and genero like "%'.$genero->genero.'%"')->result();
			...
			$arrayControl = array();
			array_push($arrayControl, 5000);
			array_push($arrayControl, $data);
			...
			$arrayControl = array();
			array_push($arrayControl, 456);
			$query = $this->db->query('select * from participante_actividade where id_actividade='.$id_actividade)->result();

			array_push($arrayControl, $query);
			return $arrayControl;
		}
	}
	...
	function getInfoCabecalhoLista($id){

		$query = $this->db->query('select director_prova,cidade_campeonato from campeonato
	...
	function getInfoCabecalhoListaProvincial($id){
	...
	function getInfoCabecalhoListaPDF($id){
      // if($data != 0){
			//
	...
	function getInfoRelatorioFinal45567($id){
	...
	function getCampeonatoClubeParti3($id){

		$query = $this->db->query('select DISTINCT(clube) from participante_campeonato where id_campeonato_fk ='.$id)->result();
	...
	function getcategoriaPeso2($idcampeonato = null){
		...
		if($idcampeonato != 0){
			...
			$query = $this->db->query('select distinct(descricao_escalao) from campeonato_escalaoetario
			inner join campeonato on campeonato.id_campeonato = campeonato_escalaoetario.id_campeonato
			where campeonato.id_associacao = '.$idAssociacao.' and campeonato_escalaoetario.id_campeonato='.$idcampeonato)->result();
			...
			foreach($query as $escalao){
				...
				$generos = $this->db->query('select * from campeonato_escalaoetario
				where id_campeonato='.$idcampeonato.' and descricao_escalao like "%'.$escalao->descricao_escalao.'%"')->result();
				...
				foreach($generos as $genero){
					...
					$categoriaPesos = $this->db->query('SELECT descricao_categoriaPeso FROM campeonato_categoriapeso
						inner join campeonato_escalaoetario on campeonato_escalaoetario.id_campeonato_escalaoetario = campeonato_categoriapeso.id_campeonatoescalao
						where campeonato_escalaoetario.descricao_escalao="'.$escalao->descricao_escalao.'" and campeonato_escalaoetario.genero="'.$genero->genero.'" and campeonato_escalaoetario.id_campeonato='.$idcampeonato)->result();
	...
	function getsistemadeluta($sistemaLuta){
       ...
       $query = $this->db->query('SELECT descricaosistemaluta FROM campeonato_sistemadeluta
				inner join campeonato on campeonato.id_campeonato = campeonato_sistemadeluta.id_campeonato
	 			where campeonato.id_associacao = '.$idAssociacao.' and campeonato_sistemadeluta.id_campeonato ='.$sistemaLuta)->result();
	...
	public function verificarBilheteDuplicado($bilhete){
		$query = $this->db->query('select * from participante_campeonato where bilhete_identidade like "%'.$bilhete.'%"')->result()[0];
		if(empty($query)){
			return false;
		}else{
			return true;
	...
	public function layoutParticipantesFinalGraduacao($id = null){
		$query = $this->db->query("select * from participante_actividade where id_actividade=".$id)->result();
		return $query;
	}
	...
	function layoutClassificacaoFinal($idcampeonato = null){
		...
		$query = $this->db->query('select distinct(escalao) from participante_campeonato
		where id_campeonato_fk='.$idcampeonato)->result();

		$data = array();

		foreach($query as $escalao){
			if($escalao->escalao != null){
				$data["$escalao->escalao"] = array(
					'escalao' => $escalao->escalao,
					'generos' => array()
				);

				$generos = $this->db->query('select distinct(genero) from participante_campeonato
				where escalao like "%'.$escalao->escalao.'%" and id_campeonato_fk='.$idcampeonato)->result();

				foreach($generos as $genero){

					$data["$escalao->escalao"]["generos"]["$genero->genero"] = array(
						'genero' => $genero->genero,
						'categoriaPesos' => array()
					);

					$categoriaPesos = $this->db->query('SELECT * FROM campeonato_categoriapeso WHERE id_campeonatoescalao in
					(SELECT id_campeonato_escalaoetario FROM campeonato_escalaoetario WHERE id_campeonato='.$idcampeonato.' and descricao_escalao like "%'.$escalao->escalao.'%" and genero like "%'.$genero->genero.'%")')->result();

					foreach($categoriaPesos as $categoriaPeso){

						$data["$escalao->escalao"]["generos"]["$genero->genero"]["categoriaPesos"]["$categoriaPeso->descricao_categoriaPeso"] = array(
						...
						// echo json_encode($classificacaoFinal->name_participante);
								$data["$escalao->escalao"]["generos"]["$genero->genero"]["categoriaPesos"]["$categoriaPeso->descricao_categoriaPeso"]["classificacaoFinals"]["$classificacaoFinal->posicaoDes"] = array(
									...
									'bilhete_identidade' => $classificacaoFinal->bilhete_identidade,
	...
	//Classificacao por clubes

	function layoutClassificacaoClubeFinal($idcampeonato = null){

		$query = $this->db->query('select DISTINCT(clube) from participante_campeonato
		where participante_campeonato.id_campeonato_fk ='.$idcampeonato)->result();
		...
		foreach($query as $provincia2){
			...
			$medalhas = $this->db->query('select count(posicaoDes) as posicaoDes from posicaoclassificacao
			inner join participante_campeonato on participante_campeonato.id_participante_campeonato = posicaoclassificacao.id_participante
			where clube like "%'.$provincia2->clube.'%" and participante_campeonato.id_campeonato_fk ='.$idcampeonato)->result();

			foreach($medalhas as $medalha){

				array_push($data, array("nome_provincia" => $provincia2->clube,
										"nome_medalha" => $medalha->posicaoDes
									)
								);
			...
			$medalhas2 = $this->db->query('select posicaoDes,count(*) as Qtd from posicaoclassificacao
								inner join participante_campeonato on participante_campeonato.id_participante_campeonato = posicaoclassificacao.id_participante
								where clube like "%'.$provincia2->clube.'%" and participante_campeonato.id_campeonato_fk ='.$idcampeonato.' GROUP BY posicaoDes')->result();
	...
	function getEscalaogenero555($id_Escalao = null, $id_campeonato = null){
		...
		$query = $this->db->query('select distinct(genero) from campeonato_escalaoetario where descricao_escalao = "'.$id_Escalao.'" and id_campeonato ='.$id_campeonato)->result();
		return $query;
	...
	function getInfoParticipanteCampeonatoLista($id = null){

		$query = $this->db->query('select * from participante_campeonato where participante_campeonato.id_campeonato_fk ='.$id)->result();
