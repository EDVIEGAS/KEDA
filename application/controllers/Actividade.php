...
class Actividade extends Qr_code_generate
{
	...
	public function GerarPDFCertificado()
	{
	...
	public function BuscaByBilheteMestre()
  	{
        ...
        $numero = $this->mestre_model->getAllMestreByBilheteCampProvincial($idAssociacao, $this->input->post('numero'));
				...
				if(count($numero) == 0){
					...
					$dado = [
						...
						'academia' => $numero[0]->nome_academia
					];
	...
	public function BuscaByBilheteMestreGraduacao()
  {
		...
		$numero = $this->mestre_model->getAllMestreByBilheteGraduacao($this->input->post('numero'));
 ...
 var_dump($dados);
	...
	public function Consultar($idEvento = null, $idAssociacao = null)
  	{
    	...
    	if ($this->session->userdata('valida')):
			...
			$data['infoGraduacao'] = $this->verSeminarioInfo($idEvento, $palavra1[0]);

			// var_dump($palavra1[0]);

			$data['controlTI45'] = $this->campeonato_model->getcampeonato($idEvento,$palavra1[0],$idcampeonato);
			// var_dump($data['controlTI45']);
	...
	public function getGraduacaoFixasSeminario(){
		...
		$id_actividade = $this->input->post('idEvento2');
		$graduacao_actual = $this->input->post('graduacao_actual');
		$rest = substr($graduacao_actual, 0);

		$retorno5 = $this->seminarios_outros->BuscarInformGraduacao2($id_actividade);
	...
	public function verSeminarioInfo($idEvento = null, $nomeActividade){

		$this->load->model('seminarios_outros');

		$id = $this->input->post('idEvento2');

		if($nomeActividade == "Seminario"){
			$query = $this->db->query('select id_graduacaofederacao from graduacaofederacao where id_actividade='.$idEvento)->result()[0];
			$retorno = $query->id_graduacaofederacao;

			if(!empty($query)){

				$retorno2 = $this->seminarios_outros->BuscarInformGraduacao($retorno);
				return $retorno2;
			}else{
	...
	public function listarParticipaanteListaFinalizada($idevento = null){
		...
		$idEvento = $this->input->post('idEvento5');

		$dados = $this->seminarios_outros->setAllParticipanteSeminario2($idEvento);
