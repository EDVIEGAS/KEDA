...
//-----------------Funcao gerar pdf listar Participante Campeonato---------------------------------------------------
...
//---------------------------------------------------Gerar PDF de diplomas do campeoanto-------------------------------------------------------------------------//

public function GerarPDFDiplomasCampeonato($id_campeonato)
{
	$this->load->model('campeonato_model');
	$this->load->model('seminarios_outros');

	// $data = $this->actividadePDF($idEvento);

	// $dado["participantes"] = $this->campeonato_model->setAllParticipante($id_campeonato);
	$dado["participantesCabecalho"] = $this->campeonato_model->getInfoCabecalhoListaPDF($id_campeonato);
	$dado['layoutClassificacaoFinalCertificado'] = $this->campeonato_model->layoutClassificacaoFinal99($id_campeonato);
	...
	$this->load->view('pdfs/diploma.php',$dado);
...
public function GerarPDFListarParticipantesCampeonato($id_campeonato)
{
	$this->load->model('campeonato_model');
	...
	$this->load->view('pdfs/ListarParticipanteCampeonato.php',$dado);
	...
	$dompdf->render();
	$dompdf->stream('Lista do Campeonato.pdf', ["Attachment" => false]);

}
//-----------------Funcao gerar pdf listar Participante Campeonato---------------------------------------------------
public function GerarPDFListarParticipantesCampeonatoProvincial($id_campeonato)
{
	$this->load->model('campeonato_model');
	...
	$dado["participantes"] = $this->campeonato_model->setAllParticipante($id_campeonato);
	$dado["participantesCabecalho"] = $this->campeonato_model->getInfoCabecalhoListaProvincial($id_campeonato);

	// var_dump($dado["participantesCabecalho"]);
...
public function organizarCategoria(){
  ...
  public function addCampeonatoNacional(){
    ...
    if($this->session->userdata('valida')):
      ...
      $this->form_validation->set_rules('bilhete', 'Bilhete de Identidade', 'required','numeric',
        ...
        $dado2 = [
          ...
          'localdaprova' => $this->input->post('localDaProva')
          // 'horaFim' => $this->input->post('campeonatoNome')
        ];

        $cadastro1 = $this->campeonato->setCampeonato_nacional($dado2);

				// redirect(base_url('GerirNacional/459'));
				if($cadastro1 != 0):
					...
					$this->campeonato->updateControlTelaNacional($cadastro,$dados);
					$this->session->set_flashdata('sucesso_nacional', '<div class="alert alert-success alert-dismissible" style="background:#00a65a;border-color:#008d4c;"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h7 style="color:white"><i class="icon fa fa-check"></i> Sucesso!. O Campeonato foi adicionado.</a></h7></div>');
          echo ($cadastro);
					// var_dump($dados4);
				else:
					$this->session->set_flashdata('erro_delete_formando', '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4><i class="icon fa fa-check"></i> FALHA!</h4>Não foi possível eliminar o formando.</div>');
	...
	//Cadastrar os dados do campeonato PROVINCIAL
  ...
  public function addCampeonatoProvincial(){
    ...
    if($this->session->userdata('valida')):
      ...
      $this->form_validation->set_rules('bilhete', 'Bilhete de Identidade', 'required','numeric',
        ...
        $dados = [
          'nome_campeonato' => $this->input->post('Campeonato'),
          'id_local_campeonato' => $id_local_campeonato,
          'id_associacao' => $this->input->post('id_associacao_pro'),
          'fk_actividadeC' => $this->input->post('fk_actividadeC')
        ];
        ...
        $dado2 = [
          ...
          'edicao_campeonato' => $this->input->post('Edicao'),
          'id_campeonato' => $cadastro,
          'descricao_provincial' => $this->input->post('campeonatoNome'),
          'dataHoraInicio' => $this->convertDate($this->input->post('data_inicio')),
          'dataHoraFim' => $this->convertDate($this->input->post('data_fim')),
          'localdaprova' => $this->input->post('localDaProva')
          // 'horaFim' => $this->input->post('campeonatoNome')
        ...
        $cadastro1 = $this->campeonato->setCampeonato_provincial($dado2);
				...
				if($cadastro1 != 0):
					...
					$dados4 = [
						'id_provincial' => $cadastro,
						'id_associacao' => $this->input->post('id_associacao_pro')
					];

					$this->campeonato->updateProvincialAssociacaolAtual(1,$dados4);
					$this->campeonato->updateControlTelaNacional($cadastro,$dados);
					$this->session->set_flashdata('sucesso_nacional', '<div class="alert alert-success alert-dismissible" style="background:#00a65a;border-color:#008d4c;"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h6 style="color:white"><i class="icon fa fa-check"></i> Sucesso!. O Campeonato foi criado.</a></h6></div>');
          echo ($cadastro);
					// var_dump($cadastro);
				else:
					$this->session->set_flashdata('erro_delete_formando', '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4><i class="icon fa fa-check"></i> FALHA!</h4>Não foi possível eliminar o formando.</div>');
	...
	public function updateControlTelaNacional(){
		$this->load->model('campeonato_model','campeonato');
    ...
    if($this->session->userdata('valida')):
			...
			$this->campeonato->updateControlTelaNacional($id_campeonato,$dados);
	...
	//verifica se existe uma participante de um determinado campeonatos nacional

	public function verificarParticipanteNacional(){

		$this->load->model('campeonato_model','campeonato');

    if($this->session->userdata('valida')):

			$id_campeonato = $this->input->post('idCampeonato');


			$retorno = $this->campeonato->verificarParticipanteNacional($id_campeonato);
      if(count($retorno) == 0){
				echo 1;
			}else{
				echo 2;
	...
	public function verificarclassificacaoBlankProvincial(){
    ...
    if($this->session->userdata('valida')):
			...
			$retorno = $this->campeonato->layoutClassificacaoFinal($id_campeonato);
      if(count($retorno) == 0){
				echo 1;
			}else{
				echo 2;
	...
	//verifica se existe classificacao definida......................................................................................

	public function verificarclassificacaoBlankNacional(){
  ...
  public function CampeonatoGerir200($valor = NULL)
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $this->template->load('template','ver/Campeonato_Gerir',$data);
  ...
  public function CampeonatoGerir400($valor = NULL)
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $this->template->load('template','ver/Campeonato_Gerir_africano',$data);
  ...
  public function ListarCampeonatoMundial()
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        // $data['categoria'] = $this->cru->get_categoria_atleta();
        $data['mundial'] = $this->campeonato_model->getAllMundial();
        // $data['categoria'] = $this->campeonato_model->setAllcategoria();
        // $data['campeonato'] = $this->campeonato_model->getIdCampeonato($id,$nome);
        $data['menus'] = $this->Menu_model->obterMenus($this->session->userdata('id_role'));
        ...
        $this->template->load('template','listar/listar_mundial',$data);
  ...
  public function ListarCampeonatoAfricano()
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $data['africano'] = $this->campeonato_model->getAllAfricano();
        ...
        $this->template->load('template','listar/listar_africano',$data);
	...
	//Metodo listar Campeonato Nacional........................................................................................

  public function ListarCampeonatoNacional()
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $data['nacional'] = $this->campeonato_model->getAllNacional();

				// var_dump($data['nacional']);
        ...
        $this->template->load('template','listar/listar_nacional',$data);
  ...
  public function ListarCampeonatoProvincial()
  {
        ...
        $query = $this->db->query("SELECT id_associacao_pro, descricao_associacao FROM associacao_provincia WHERE descricao_associacao = '$descricao'")->result();
		    ...
		    if($query != NULL){
		      ...
		      $id_associacao_pro = '';
				...
				$data['provincial'] = $this->campeonato_model->getAllProvincial($id_associacao_pro);

        $this->template->load('template','listar/listar_provincial',$data);
  ...
  public function verAllParticipanteMundial($id)
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $data['Participantes_mundial'] = $this->campeonato_model->getAllParticipantesMundialId($id);

        $this->campeonato_model->atualizarMundialActual(1,$dado);
        ...
        $this->template->load('template','ver/verParticipantesMundial',$data);
  ...
  public function verAllParticipanteAfricano($id)
  {
      ...
      if ($this->session->userdata('valida')):
        ...
        $data['Participantes_africano'] = $this->campeonato_model->getAllParticipantesAfricanoId($id);

        $this->campeonato_model->atualizarAfricanoActual(1,$dado);
        ...
        $this->template->load('template','ver/verParticipantesAfricano',$data);
  ...
  public function getParticipantes(){
    ...
    $participantes = $this->campeonato_model->setAllParticipante($idcampeonato);
  ...
  public function getInfoCabecalhoLista(){
    ...
    $participantes = $this->campeonato_model->getInfoCabecalhoListaProvincial($idcampeonato);
  ...
  public function getInfoCabecalhoListaPDF($idEvento = null){
  ...
  public function getInfoRelatorioFinal345(){

    $this->load->model('campeonato_model');

    $idcampeonato = $this->input->post('idcampeonato');

    $participantes = $this->campeonato_model->getInfoRelatorioFinal45567($idcampeonato);
  ...
  public function getCampeonatoClubesnParti(){

    $this->load->model('campeonato_model');

    $idcampeonato = $this->input->post('idcampeonato');

    $participantes = $this->campeonato_model->getCampeonatoClubeParti3($idcampeonato);
  ...
  public function setParticipanteEvento(){
		...
		$dado = [
			'id_participante_fk' => $this->input->post('id'),
    ...
    // $control_registro = $this->campeonato_model->verificarBilheteDuplicado($this->input->post('bilhete'));

    // if($control_registro == true){
    //   echo 89876;
    // }else{
  ...
  public function setParticipanteCompEventoProvincial(){

    $this->load->model('campeonato_model');

    $dados =  $this->input->post('dados2');
    ...
    $existeTable = [];
          ...
          $resposta909 = $this->campeonato_model->verificaParticipanteCampeonato($dados['bilhete'], $idcampeonato);

          if(count($resposta909) != 0){
            array_push($existeTable,array("nome" => $dados['nome'], "bilhete" => $dados['bilhete'],));
          }else{

            $dado = [
              'id_participante_fk' => $dados['id_mestre'],
              'id_campeonato_fk' => $idcampeonato,
              'bilhete_identidade' => $dados['bilhete'],
              'categoria' => $dados['peso'],
              'funcao' => 'Competidor',
              'nome_participante' => $dados['nome'],
              'imagem' => $dados['imagem'],
              'provincia' => $dados['provincia'],
              'genero' => $dados['genero'],
              'dataNascimento' => $dados['data_nascimento'],
              'clube' => $dados['nome_academia'],
              'escalao' => $escalao,
              'categoriaPeso' => $this->verificarCategoria($dados['peso'],$dados['genero'],$escalao)
            ];
  ...
  public function setVParticipanteEventoArbitro(){
    ...
    $dados =  $this->input->post('dados');
    		...
    		$idcampeonato = $dados[count($dados)-1];
        ...
        for($i = 0;$i < count($dados)-1;$i++){
          ...
          if(count($resposta909) != 0){
            array_push($retornoExistente,array("nome" => $dados[$i]['nome'], "bilhete" => $dados[$i]['n_bilhete'],));
          }else{
            ...
            $resposta = $this->campeonato_model->setParticipanteCampeonato($dado);
				...
				if(count($retornoExistente) == 0){
					echo 0;
				}else{
					echo json_encode($retornoExistente);
				}
  ...
  public function setVParticipanteEventoTreinador(){
    ...
    $idcampeonato = $dados[count($dados)-1];
        ...
        for($i = 0;$i < count($dados)-1;$i++){
					...
					if(count($resposta909) != 0){
						array_push($retornoExistente2,array("nome" => $dados[$i]['nome'], "bilhete" => $dados[$i]['n_bilhete'],));
					}else{
            ...
            $resposta = $this->campeonato_model->setParticipanteCampeonato($dado);
				...
				if(count($retornoExistente2) == 0){
					echo 0;
				}else{
					echo json_encode($retornoExistente2);
				}
  ...
  function setInformacaoCamp(){
        ...
        $query = $this->db->query('select id_nacional from camp_nacional_atual')->result()[0];
        $retorno = $query->id_nacional;
        ...
        if(!empty($dados['sistemaLutaFinal'])){

          for($i=0;$i < count($dados['sistemaLutaFinal']);$i++){
            ...
            $dado = [
              ...
              'descricaosistemaluta' => $dados['sistemaLutaFinal'][$i]
            ];

            $retorno77 = $this->campeonato_model->verificarCampeonatoSistemaLuta($retorno,$dados['sistemaLutaFinal'][$i]);
            ...
            if(count($retorno77) != 0){
              ...
              array_push($dado99,array($dados['sistemaLutaFinal'][$i]));
            ...
            $retorno2 = $this->campeonato_model->setcampeonatoEscalao($dado);
                ...
                if(!empty($teste[$i]["categoriaPeso"])){
                  ...
                  for($j=0;$j < count($teste[$i]["categoriaPeso"]);$j++){
                    ...
                    // console.log($dado);
                    $retorno3 = $this->campeonato_model->setcampeonatoPeso($dado2);
  ...
  public function getEscalaoCampeonato(){
    ...
    if($retorno[0]->descricao_escalao == "9056as34"){
      echo $retorno[0]->descricao_escalao;
    }else{
    ...
    $retorno = $this->campeonato_model->buscarAtletaClassificacao($genero, $categoriaPeso, $id_campeonato_fk, $escalao);
      ...
      if($retorno != 25){
        ...
        foreach($retorno as $retorno){
          ...
          $mostra .= '<option value="0"> escolhe uma opcao </option>';
        ...
        $mostra .= '</table>';
				...
				array_push($conteudo, array('total' => $conteudo66,'conteudo' => $mostra));
  ...
  function ClassificacaoFinalRelatorioProvincia(){
    ...
    $mostra = "";
    $mostra .= '<div class="text-center">';
    $mostra .= '<span class="font-weight-bold">CLASSIFICACAO DAS MEDALHAS POR PROVINCIA</span>';
  ...
  function ClassificacaoFinalRelatorioClube(){

		$this->load->model('campeonato_model');

    $retornoRelatorio = ($this->campeonato_model->layoutClassificacaoClubeFinal($this->input->post('idcampeonato')));

    $mostra = "";
    $mostra .= '<div class="text-center">';
    $mostra .= '<span class="font-weight-bold">CLASSIFICACAO DAS MEDALHAS POR CLUBE</span>';
  ...
  function ClassificacaoFinalRelatorioClube2(){
    ...
    foreach($retornoRelatorio as $retornoRelatoriof){
      ...
      // if($retornoRelatorio[$i]['nome_medalha'] > $this->ordenarArray()){
        $mostra .='<span class="tsp-pb4 font-weight-bold" style="font-family: arial,sans-serif;font-weight: normal;font-size:14px"><u><b>'.$control++.'ª Lugar</b></u></span><br>';
        $mostra .='<p><span class="tsp-fr tsp-flr tsp-pb4 font-weight-bold" style="font-family: arial,sans-serif;font-weight: normal;"><u>'.$retornoRelatoriof['nome_provincia'].'</u> - </span><span>'.$retornoRelatoriof['nome_medalha'].' Medalhas</span></p>';
        $i=0;
        $a=0;
	...
	function registrarMundialAfricano(){

		$this->load->model('campeonato_model','campeonato');

		$pais = $this->input->post('paisLocal');
   ...
   $retorno = $this->campeonato->GravarMundial($dado);

	 $dado2 = [
		 'id_mundial' => $retorno
	 ];

	 $retorno4 = $this->campeonato->atualizarMundialActual(1,$dado2);
	...
	function atualizarCampeonatoAtual77(){
		$this->load->model('campeonato_model','campeonato');
	...
	function atualizarCampeonatoAtual88(){
		$this->load->model('campeonato_model','campeonato');
		...
		$retorno4 = $this->campeonato->atualizarAfricanoActual(1,$dado2);

	}

	function registrarAfricano(){
   ...
   $retorno = $this->campeonato->GravarAfricano($dado);
	...
	function pegarCampeonatoMundialAtual(){
		$this->load->model("campeonato_model",'campeonato');

		$retorno = $this->campeonato->getMundialActual();
		echo($retorno);
	}

	function pegarCampeonatoAfricanoAtual(){
		$this->load->model("campeonato_model",'campeonato');

		$retorno = $this->campeonato->getAfricanoActual();
		echo($retorno);
	}

	function EliminarParticipanteCampeonatoMundialAtual(){
		$this->load->model("campeonato_model",'campeonato');

		$id_participante = $this->input->post('id_participante');

		$retorno = $this->campeonato->EliminarParticipanteCampeonatoMundialAtual($id_participante);
		if($retorno == 1){
			echo 1;
		}else{
			echo 2;
	...
	function EliminarCampeonatoAfricano(){
		$this->load->model("campeonato_model",'campeonato');

		$id_africano = $this->input->post('id_africano');

		$retorno = $this->campeonato->EliminarCampeonatoAfricano55($id_africano);
	...
	function EliminarCampeonatoANacional(){
		$this->load->model("campeonato_model",'campeonato');

		$idcampeonatoNacional = $this->input->post('idcampeonato');

		$retorno = $this->campeonato->EliminarCampeonatoNacional($idcampeonatoNacional);

		// $this->session->set_flashdata('sucesso_mundial', '<div class="alert alert-success alert-dismissible" style="background:#00a65a;border-color:#008d4c;fonte_"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h7 style="color:white"><i class="icon fa fa-check"></i> Sucesso!. Academia'.$this->input->post('nome_academia').' adicionado.</h7></div>');
		// redirect('listaMundial');
	}

	function verificarParticipanteMundial(){
		$this->load->model("campeonato_model",'campeonato');

		$id_mundial = $this->input->post('id_mundial');

		$retorno = $this->campeonato->verificarCampeonatoMundial55($id_mundial);
  ...
  function EliminarCampeonatoMundial(){
    $this->load->model("campeonato_model",'campeonato');

		$id_mundial = $this->input->post('id_mundial');

    $retorno = $this->campeonato->EliminarCampeonatoMundial55($id_mundial);
	...
	function verificarParticipanteAfricano(){
		$this->load->model("campeonato_model",'campeonato');

		$id_africano = $this->input->post('id_africano');

		$retorno = $this->campeonato->verificarCampeonatoAfricano55($id_africano);
	...
	function EditarParticipanteCampeonatoMundialAtual(){
		$this->load->model("campeonato_model",'campeonato');

		$id_participante = $this->input->post('id_participante');

		$dados = [
			'posicao' => $this->input->post('classificacao')
		];

		$retorno = $this->campeonato->atualizarParticipanteCampeonatoMundial($id_participante,$dados);
		if($retorno == 1){
			echo 1;
		}else{
			echo 2;
	...
	function setParticipanteMundial(){
		$this->load->model("campeonato_model","campeonato");
		$dados = [
			'id_mundial' => $this->input->post('id_mundial'),
			'nome_pessoa' => $this->input->post('nome_pessoa'),
			'n_bilhete' => $this->input->post('n_bilhete'),
		...
		$retorno88 = $this->campeonato->verificarParticipantesMundialBilhete($this->input->post('n_bilhete'),$this->input->post('id_mundial'));

		if($retorno88 == 55){
			$retorno = $this->campeonato->setPParticipantesMundial($dados);
	...
	//funcao para adicionar participantes no africano.....................................................................................

	function setParticipanteAfricano(){
		$this->load->model("campeonato_model","campeonato");
		...
		$retorno88 = $this->campeonato->verificarParticipantesAfricanoBilhete($this->input->post('n_bilhete'),$this->input->post('id_africano'));

		if($retorno88 == 55){
			$retorno = $this->campeonato->setPParticipantesAfricano($dados);
	...
	function getAllParticipanteMundial(){
		$this->load->model("campeonato_model","campeonato");

		$id = $this->input->post('dados');

		$retorno = $this->campeonato->getAllParticipantesMundial($id);

		echo json_encode($retorno);
	}


	public function gerirCampeonatoNacional($id_campeonato = null)
	{
		    	...
		    	if ($this->session->userdata('valida')):
							...
							// $this->controlIdDetalhes($id_campeonato);

							$query = $this->db->query("SELECT id_nacional FROM camp_nacional_atual")->result()[0];
							$idcampeonato = $query->id_nacional;
// var_dump($idcampeonato);
							$data['provincia'] = $this->Provincia->getAllProv();
							$data['menus'] = $this->Menu_model->obterMenus($this->session->userdata('id_role'));
							$data['valorControl'] = $this->controlTelas44($idcampeonato);
							$data['idcampeonato'] = $idcampeonato;
							$data['teste'] = $this->campeonato_model->getcategoriaPeso2($idcampeonato);
							$data['sistemaLuta2'] = $this->campeonato_model->getsistemadeluta($idcampeonato);
							$data['participanteEPcamp'] = $this->campeonato_model->setAllParticipante2($idcampeonato);
							$data['layoutClassificacaoFinal'] =$this->campeonato_model->layoutClassificacaoFinal($idcampeonato);
							// var_dump($data['layoutClassificacaoFinal']);
							// $data['layoutParticipantesAprovadosGraduacao'] =$this->seminarios_outros->setAllParticipanteSeminario2($idEvento);
							...
							$data['getCapNacionalInfo'] =$this->campeonato_model->getCapNacionalInfo($idcampeonato);

							// var_dump($data['getCapNacionalInfo']);
					...
					//
							$this->template->load('template','ver/gerir_campeonato_nacional',$data);
	...
	public function gerirCampeonatoProvincial($id_campeonato = null)
	{
					// $this->auth->check_logged(0,$this->router->class , $this->router->method);
		    	...
		    	if ($this->session->userdata('valida')):
							...
							$query = $this->db->query("SELECT id_provincial FROM camp_provincial_atual")->result()[0];
							$idcampeonato = $query->id_provincial;

							$this->controlIdDetalhes($idcampeonato);
// var_dump($idcampeonato);
							$data['provincia'] = $this->Provincia->getAllProv();
							$data['menus'] = $this->Menu_model->obterMenus($this->session->userdata('id_role'));
							$data['valorControl'] = $this->controlTelas44($idcampeonato);
							...
							$data['sistemaLuta2'] = $this->campeonato_model->getsistemadeluta($idcampeonato);
							$data['participanteEPcamp'] = $this->campeonato_model->setAllParticipante2($idcampeonato);
							$data['participanteEPcamp5'] = $this->campeonato_model->setAllParticipante($idcampeonato);
							$data['totalParticipante'] = $this->campeonato_model->getAllParticipanteTotal($idcampeonato);
							$data['totalParticipanteMasc'] = $this->campeonato_model->getAllParticipanteTotalMasculino($idcampeonato);
							$data['totalParticipanteFem'] = $this->campeonato_model->getAllParticipanteTotalFemenino($idcampeonato);
							$data['layoutClassificacaoFinal'] =$this->campeonato_model->layoutClassificacaoFinal($idcampeonato);
							...
							// echo '<br>';
							// echo '<br>';
							// var_dump($data['layoutClassificacaoFinalCertificado']);
					//
					// $data['evento'] = $this->calendario->getActividadeById($idEvento);
							$data['escalao'] = $this->campeonato_model->getEscalao();
							$data['categoriaIdade'] = $this->campeonato_model->getCategoriaIdade();
					...
					// $data['sistemaL666uta'] = $this->campeonato_model->layoutClassificacaoProvinciaFinal(1);
					//
							$this->template->load('template','ver/gerir_campeonato_provincial',$data);
...
public function infoCampProvincial(){

	$this->load->model('campeonato_model');

	$datateste = $this->campeonato_model->getcategoriaPeso2($idcampeonato);
	$datasistemaLuta2 = $this->campeonato_model->getsistemadeluta($idcampeonato);

}


//ADICIONAR NOVO CAMPEONATO NACIONAL.........................................................................................
	public function NovoCampeonatoNacional()
	{
					// $this->auth->check_logged(0,$this->router->class , $this->router->method);
		    	...
		    	if ($this->session->userdata('valida')):
					...
					$query = $this->db->query("select controlTela from campeonato where id_campeonato =".$idcampeonato)->result();
		      	...
		      	$controlTela = $query[0]->controlTela;
						...
						$controlTela = null;
							...
							if($controlTela == 1 || $controlTela == 2 || $controlTela == 3){
								redirect('campeonato/gerirCampeonatoNacional');
							}else{
								$this->template->load('template','formularios/form_campeonato_nacional',$data);
							}
					//
...
// cadastrar campeonato Provincial
	public function NovoCampeonatoProvincial()
	{
					// $this->auth->check_logged(0,$this->router->class , $this->router->method);
	        ...
	        $query = $this->db->query("SELECT id_associacao_pro, descricao_associacao FROM associacao_provincia WHERE descricao_associacao = '$descricao'")->result();
			    ...
			    if($query != NULL){
			      ...
			      $data['id_associacao_pro'] = '';
					...
					var_dump($controlTela);

					if($controlTela == 1 || $controlTela == 2 || $controlTela == 3){
						redirect('campeonato/gerirCampeonatoProvincial');
					}else{
						$this->template->load('template','formularios/form_campeonato_provincial',$data);
					}
					// $this->template->load('template','formularios/form_campeonato_provincial',$data);
...
public function controlTelas44($id = null){

	$query = $this->db->query('SELECT controlTela FROM campeonato where id_campeonato ='.$id)->result()[0];
	return $query->controlTela;

}

public function controlIdDetalhes($id = null){
...
public function controlIdDetalhes5($id = null){

	$this->load->model("campeonato_model","campeonato");

	$dados4 = [
		'id_nacional' => $this->input->post('id_nacional')
	];
	...
	function getAllParticipanteAfricano(){
		$this->load->model("campeonato_model","campeonato");

		$id = $this->input->post('dados');

		$retorno = $this->campeonato->getAllParticipantesAfricano($id);
