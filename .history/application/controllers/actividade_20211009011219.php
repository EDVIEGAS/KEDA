...
// var_dump($numero);
	...
	public function verSeminarioTipo(){
		...
		$this->load->model('seminarios_outr');
	...
	public function verSeminarioInfo(){

		$id = $this->input->post('idEvento2');

		$query = $this->db->query('select id_graduacaofederacao from graduacaofederacao where id_actividade='.$id)->result();
		$retorno = $query->id_graduacaofederacao;

		if(!empty($query)){
			$query2 = $this->db->query('select id_graduacaofederacao from graduacaofederacao where id_actividade='.$id)->result();
			$retorno = $query2->id_graduacaofederacao;
		}
